...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-6">
            <table>
    <form  id="dataForm" method="post" action="/suplements">
        @csrf
            <tr>
                <td>tahun:</td>
                <td><input type="text" name="tahun" value="{{$suplement->tahun}}" readonly></td>
            </tr>
            <tr>
                <td>Nomor Bukti:</td>
                <td><input type="text" name="nomor_bukti" value="{{$suplement->nomor_bukti}}"></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td colspan="2"><a class="btn btn-primary" href="/suplements">selesai</a>
                </td>
            </tr>
    </form>
...
<script>
    function tambahData() {
        ...
        $.ajax({
            ...
            url: '/suplements', // URL endpoint yang dituju
...
<script>
   $(document).ready(function () {
    function getData() {
        $.ajax({
            url: '/api/suplements/{{$suplement->nomor_bukti}}',
            type: 'GET',
            dataType: 'json',
    ...
    function editFunction(item) {
        $.ajax({
        type: "GET",
        url: "/suplements/" + item.id + "/edit", // Ganti dengan URL yang sesuai
        success: function(response) {
            console.log(response);
            ...
            $('#dataForm').attr('action', '/suplements/' + item.id);
        },
        error: function(error) {
...
function deleteFunction(item) {
      $.ajax({
        type: 'get',
        url: '/suplements/delete/' + item.id, // Ganti dengan URL yang sesuai
        success: function (response) {
            // console.log(response);
